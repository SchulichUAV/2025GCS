...
#     for easting_drone_temp, northing_drone_temp, agl, x_pix, y_pix, yaw, pitch, roll, position_std in zip(easting_drone, northing_drone, vectors['rel_alt'], vectors['x'], vectors['y'], vectors['yaw'], vectors['pitch'], vectors['roll'], vectors['position_uncertainty']):
...
#         obs_std.append(position_std / 1000)  # Convert mm to m


#     #--------------------------------------------------Adjustment-----------------------------------------------------
#     easting_target_est, northing_target_est = parametric_adjustment(np.array(easting_drone), np.array(northing_drone), np.array(agl_drone), np.array(easting_target), np.array(northing_target), np.array(obs_std))
...
#     lat, lon = parametric_model("C:/Users/mfles/OneDrive - University of Calgary/SUAV/Model/savedCoordsWeight.json")
